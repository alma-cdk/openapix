...
export abstract class Integration implements IBaseIntegration {
  ...
  private resolveResponses(
    props: IntegrationProps,
  ): XAmazonApigatewayIntegrationResponses | undefined {
    const responses: XAmazonApigatewayIntegrationResponses = {};
    props.options?.integrationResponses?.forEach((resp) => {
      const selectionPattern = this.resolveSelectionPattern(
        props.options!.integrationResponses!,
        resp,
      );
   ...
   * resp.statusCode
   */
  private resolveSelectionPattern(
